 ...
 * @returns 向上取整后的结果
 */
export declare function roundUp(num: number, N?: number): number;
/**
 * 安全的向下取整
 * @param num 待处理的数字
 * @param N 保留的小数位数（可正可负，默认为0）
 * @returns 向下取整后的结果
 */
export declare function roundDown(num: number, N?: number): number;
/**
 * 安全的四舍五入
 * @param num 待处理的数字
 * @param N 保留的小数位数（可正可负，默认为0）
 * @returns 四舍五入后的结果
 */
export declare function round(num: number, N?: number): number;
/** 修正数字到安全精度范围 */
